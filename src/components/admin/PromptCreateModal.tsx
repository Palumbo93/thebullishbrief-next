...
interface PromptCreateModalProps {
  onClose: () => void;
  onCreate: (data: Partial<Prompt> & { fields: Partial<PromptField>[] }) => void;
}
...
export const PromptCreateModal: React.FC<PromptCreateModalProps> = ({ onClose, onCreate }) => {
  ...
  useEffect(() => {
    const hasChanges = 
      ...
      formData.intended_llm !== '' ||
      formData.original_credit !== '' ||
      fields.length > 0;
        ...
        <div style={{
          ...
          <div>
            ...
            <p style={{
              ...
              Create a new AI prompt template with dynamic fields
            </p>
          </div>
        </div>
